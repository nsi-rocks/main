  ...
  <UDashboardPanel id="eleves-1" resizable :min-size="20" :max-size="80" :default-size="30">
    <template #header>
      <UDashboardNavbar title="Images">
        <template #right>
          <USelect v-model="selectedClass" :items="allClasses" placeholder="Classe" class="w-40" />
        </template>
      </UDashboardNavbar>
    </template>
    <template #body>
      <UCard v-for="eleve in filteredView" :key="eleve.id" :title="eleve.firstName + eleve.lastName">
        <template #header>
          <div class="flex items-center">
            <UIcon name="i-lucide-user" class="mr-2" />
            <span>{{ eleve.firstName }} {{ eleve.lastName }}</span>
            <UBadge variant="soft" size="lg" v-if="!eleve.teacher" class="ml-auto">{{ eleve.classes }}</UBadge>
          </div>
        </template>
        <div class="flex items-center">
