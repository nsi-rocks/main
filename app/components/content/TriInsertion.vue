<template>
  <div style="margin-bottom: 1rem;">
    ...
    <transition-group
      name="slide"
      tag="div"
      style="height: 30px; position: relative; margin-top: 2rem;"
    >
      <div
        v-for="(item, index) in numbers"
        :key="index"
        :style="{ height: 3*item.val + 'px', order: item.pos, left: item.pos*35 + 'px' }"
        class="number-bar"
...
const runInsertionSort = async () => {
  ...
  for (let i = 1; i < numbers.value.length; i++) {
    ...
    const current = numbers.value[activeIndex.value].val
    let j = i - 1
